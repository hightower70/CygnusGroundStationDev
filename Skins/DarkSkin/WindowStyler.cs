...
namespace WpfSkins
{
	public partial class WindowStyler
	{
		...
		void IconMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
		{
			if (e.ClickCount > 1)
			{
				ForWindowFromTemplate(sender, w => w.Close());
			}
			else
			{
				ForWindowFromTemplate(sender, w =>
						SendMessage(GetWindowHandle(w), WM_SYSCOMMAND, (IntPtr)SC_KEYMENU, (IntPtr)' '));
			}
		}
		...
		{
			ForWindowFromTemplate(sender, w => w.WindowState = WindowState.Minimized);
		}

		void MaxButtonClick(object sender, RoutedEventArgs e)
		{
			ForWindowFromTemplate(sender, w => w.WindowState = (w.WindowState == WindowState.Maximized) ? WindowState.Normal : WindowState.Maximized);
		}
		...
		void TitleBarMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
		{
			...
			{
				ForWindowFromTemplate(sender, w => w.DragMove());
			}
		}
		...
		public static void ForWindowFromChild(object childDependencyObject, Action<Window> action)
		{
		...
		public static void ForWindowFromTemplate(object templateFrameworkElement, Action<Window> action)
		{
