...
namespace CygnusControls
{
	...
	public class ValueConverterGroup : IValueConverter
	{
		...
		private readonly ObservableCollection<IValueConverter> m_converters = new ObservableCollection<IValueConverter>();
		private readonly Dictionary<IValueConverter, ValueConversionAttribute> m_cached_attributes = new Dictionary<IValueConverter, ValueConversionAttribute>();
		...
		{
			this.m_converters.CollectionChanged += this.OnConvertersCollectionChanged;
		}
		...
		protected virtual Type GetTargetType(int converterIndex, Type finalTargetType, bool convert)
		{
			...
			if (nextConverter != null)
			{
				ValueConversionAttribute conversionAttribute = m_cached_attributes[nextConverter];
		...
		void OnConvertersCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
		{
			...
			else if (e.Action == NotifyCollectionChangedAction.Remove)
			{
				foreach (IValueConverter converter in e.OldItems)
					this.m_cached_attributes.Remove(converter);
			}
			else if (e.Action == NotifyCollectionChangedAction.Reset)
			{
				this.m_cached_attributes.Clear();
				convertersToProcess = this.m_converters;
			}
			...
			if (convertersToProcess != null && convertersToProcess.Count > 0)
			{
				foreach (IValueConverter converter in convertersToProcess)
				{
					...
					this.m_cached_attributes.Add(converter, attributes[0] as ValueConversionAttribute);
				}
			}
		}
